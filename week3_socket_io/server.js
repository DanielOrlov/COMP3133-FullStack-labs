...
ioServer.on("connection", (socket) => {
  ...
  socket.on("feedback-from-client", (feedback) => {
    console.log(
      `Feedback received from client ${socket.id} : ${JSON.stringify(feedback)}`,
    );
  ...
  socket.on("group-message-from-client", (data) => {
    ...
    console.log(
      `Group message received from client ${socket.id} to group ${group} : ${message}`,
    );
    ...
    //   message: message,
    // });
    socket.to(group).emit("group-message-from-server", {
      user: socket.id,
      group: group,
      message: message,
    });
